...
{
    public class BanditRocketGrunt : BaseEnemy
    {
        ...
        protected virtual void Fire()
        {
            if(fireLeft)
            {
                BanditRocket projectile = objectPooler.SpawnFromPool(projectileType.ToString(), transform.position + transform.up * 0.7f + -transform.right * 0.4f, transform.rotation).GetComponent<BanditRocket>();
                projectile.Initialize(projectileSize, projectileDamage, projectileSpeed, projectileLifeTime, false);
            }
            else
            {
                BanditRocket projectile2 = objectPooler.SpawnFromPool(projectileType.ToString(), transform.position + transform.up * 0.7f + transform.right * 0.4f, transform.rotation).GetComponent<BanditRocket>();
